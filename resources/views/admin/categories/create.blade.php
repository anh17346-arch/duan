...
@section('title', __('app.add_new_category') . ' - Perfume Luxury')
...
<div class="relative max-w-4xl mx-auto px-4 py-8">
  <div class="mb-8">
    <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">{{ __('app.add_new_category') }}</h1>
    <p class="text-slate-600 dark:text-slate-400 mt-2">{{ __('app.create_new_product_category') }}</p>
  </div>
    ...
    <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <textarea id="description" name="description" rows="4"
                    ...
                    placeholder="{{ __('app.describe_category') }}">{{ old('description') }}</textarea>
          @error('description')
            <p class="text-rose-600 text-sm mt-2">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div>
          ...
          <div class="relative">
            <textarea id="description_en" name="description_en" rows="4"
                      class="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-brand-500 focus:border-transparent resize-none"
                      placeholder="{{ __('app.describe_category_en') }}">{{ old('description_en') }}</textarea>
      ...
      <div>
        ...
        <select id="status" name="status" required
                class="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-brand-500 focus:border-transparent">
          <option value="1" {{ old('status', '1') == '1' ? 'selected' : '' }} class="bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100">{{ __('app.active') }}</option>
          <option value="0" {{ old('status', '1') == '0' ? 'selected' : '' }} class="bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100">{{ __('app.hidden') }}</option>
        </select>
        @error('status')
          <p class="text-rose-600 text-sm mt-2">{{ $message }}</p>
        @enderror
